...
def run(n_drivers=100, n_timesheet=1000):
    """Run the data generation script."""
    print("Generating fake data...")
    print(f"Number of drivers: {n_drivers}")
    print(f"Number of timesheet records: {n_timesheet}")

    # Clean up old files
    if os.path.exists("files/drivers.csv"):
        os.remove("files/drivers.csv")
    if os.path.exists("files/timesheet.csv"):
        os.remove("files/timesheet.csv")

    # Generate fake data
    fake_drivers = generate_fake_drivers(n_drivers)
    fake_timesheet = generate_fake_timesheet(fake_drivers, n_timesheet)
